-- Phase 1: Database-Level Validation Functions and Triggers
...
-- Function to handle customer deletion when lead status changes
CREATE OR REPLACE FUNCTION public.handle_lead_status_change()
RETURNS TRIGGER AS $$
BEGIN
  -- If lead status changed from converted to something else, we could optionally delete the customer
  -- But for data integrity, we'll prevent this in the validation function above
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;
...
-- Add trigger to validate lead status changes
...
-- Add trigger for audit logging of conversions
